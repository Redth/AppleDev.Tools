...
public class ALToolValidateAppCommand : AsyncCommand<ALToolValidateAppCommandSettings>
{
	public override async Task<int> ExecuteAsync(CommandContext context, ALToolValidateAppCommandSettings settings)
	{
		...
		var result = await altool.ValidateAppAsync(
			settings.AppPath!,
			settings.AppType,
			settings.KeyId,
			settings.IssuerId,
...
public class ALToolValidateAppCommandSettings : FormattableOutputAppStoreConnectCommand
{
